...
function build_case()
    ...
    initial_availability = reference_capacity / 50
    initial_capacity = initial_availability / 2
...
function test_initial_capacity_benders()
    reference_capacity, initial_availability, initial_capacity, reference_capex, scaling_factor = build_case()
    filename = joinpath(@__DIR__, "test3.optbio")

    inputs, solution = OptBio.main([filename])

    calculated_investment =
        reference_capex * (initial_availability / reference_capacity)^scaling_factor -
        reference_capex * (initial_capacity / reference_capacity)^scaling_factor
...
function test_initial_capacity_deterministic()
    reference_capacity, initial_availability, initial_capacity, reference_capex, scaling_factor = build_case()
    filename = joinpath(@__DIR__, "test3.optbio")
