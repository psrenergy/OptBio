...
SELECT 
    process_id AS id,
    ROW_NUMBER() OVER (PARTITION BY process_id) AS vector_index,
    factor AS factor_input,
    product_id AS product_input
FROM 
    Input;
...
SELECT 
    process_id AS id,
    ROW_NUMBER() OVER (PARTITION BY process_id) AS vector_index,
    factor AS factor_output,
    product_id AS product_output
FROM 
    Output;
...
INSERT INTO Product_new SELECT * FROM Product;
...
INSERT INTO Process_new SELECT * FROM Process;
